...
import ArcBriefShowcase from "./ArcBriefShowcase";
    ...
    <section className="panel p-6 md:p-8" id="projects">
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-2">
        {data.map((project, index) => {
          const card = (
            <article className="flex h-full flex-col rounded-2xl border border-line/80 bg-white/90 p-5">
          ...
          if (project.name === "ArcBrief" && project.gallery?.length) {
            return (
              <ArcBriefShowcase
                key={project.name}
